...
final class SyncPageBlocksHandler
{
    ...
    public function handle(SyncPageBlocksCommand $command): array
    {
        ...
        foreach ($command->blocks as $block) {
            $this->registry->validate($block['type'], $block['payload']);
        }

        $this->pages->syncBlocks($id, $command->blocks);
        $this->cache->bump();
