...
namespace App\Application\Page\RestorePage;
...
final class RestorePageHandler
{
    ...
    public function handle(RestorePageCommand $command): array
    {
        $id = PageId::fromString($command->pageId);
        if ($this->pages->findById($id, true) === null) {
